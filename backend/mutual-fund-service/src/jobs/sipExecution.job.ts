...
import * as sipService
  from "../services/sip.service.js";
...
export const sipExecutionJob =
  cron.schedule(
    "0 9 * * *",
    async () => {

      console.log(
        "Running SIP Execution Job..."
      );
      ...
      try {
        ...
        for (const sip of executableSips) {
          ...
          try {
            ...
            console.log(
              `SIP Executed Successfully: ${sip.id}`
            );

          } catch (error: any) {

            console.error(
              `SIP Execution Failed: ${sip.id}`,
              error.message
            );
