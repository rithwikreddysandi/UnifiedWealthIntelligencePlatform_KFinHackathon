...
export const createHolding = async (payload: CreateInvestorHoldingDTO) => {
  ...
  const holding = await holdingRepository.createHolding(payload);
...
export const getHoldingById = async (id: string) => {
  return await holdingRepository.getHoldingById(id);
};

export const getInvestorHoldings = async (investorId: string) => {
  const cacheKey = `${CACHE_KEYS.INVESTOR_HOLDINGS}:${investorId}`;
  ...
  const holdings = await holdingRepository.getInvestorHoldings(investorId);

  const updatedHoldings = await Promise.all(
    holdings.map(async (holding: any) => {
      const fund = await fundRepository.getFundById(holding.fund_id);

      const currentNav = Number(fund.current_nav);
...
export const updateHolding = async (
  ...
  const existingHolding = await holdingRepository.getHoldingById(id);

  if (!existingHolding) {
    throw new Error("Holding not found");
  }

  const updatedHolding = await holdingRepository.updateHolding(id, payload);
...
export const deleteHolding = async (id: string) => {
  ...
  const deletedHolding = await holdingRepository.deleteHolding(id);
...
export const getInvestorPortfolioSummary = async (investorId: string) => {
  ...
  for (const holding of holdings) {
    ...
    totalInvestment += Number(holding.invested_amount);
  ...
  const totalReturnPercentage = calculateReturnPercentage(
    totalInvestment,
    totalCurrentValue,
  );
